...
function startOfWeek(d) {
  const x = new Date(d);
  x.setHours(0,0,0,0);
  // Week starts Monday, like the figma (Mon ... Sun)
  const day = x.getDay();
...
function seedData() {
  ...
  const tuesKey = dateKey(addDays(weekStart, 1)); // Tues = mon+1
...
// ──────────────────────────────────────────────────────────────────────
function Toolbar({ tab, setTab, search, setSearch }) {
  return (
...
// TODO TAB
// ──────────────────────────────────────────────────────────────────────
function TodoView({ state, setState, search, requestConfirm, togglePinActive, onOpenTrash }) {
  ...
  const addUnfiledNote = () => {
    ...
    setState(s => ({
      ...s,
      notes: [{ id, folderId: null, title: "New note", body: "", pinned: false, done: false, scheduled: null }, ...s.notes],
      activeNoteId: id,
    }));
  };
  ...
  const addNoteInFolder = (fid) => {
    ...
    setState(s => ({
      ...s,
      notes: [...s.notes, { id, folderId: fid, title: "New note", body: "", pinned: false, done: false, scheduled: null }],
      activeNoteId: id,
  ...
  const deleteActiveNote = () => {
    ...
    setState(s => ({
      ...s,
      notes: s.notes.filter(n => n.id !== activeNote.id),
      activeNoteId: s.notes.find(n => n.id !== activeNote.id)?.id || null,
    }));
  };
  ...
  return (
    <div className="editor-inner">
      <div className="editor-meta">
        ...
        {note.scheduled && (
          <span className="meta-pill dim">Scheduled · {fromDateKey(note.scheduled.day).toLocaleDateString(undefined,{month:"short",day:"numeric"})} · {fmtRangeFromMinutes(note.scheduled.startMin, note.scheduled.endMin)}</span>
        )}
        <button
...
// CALENDAR TAB
// ──────────────────────────────────────────────────────────────────────
function CalendarView({ state, setState, search, requestConfirm, onOpenTrash }) {
  ...
  const createNoteAt = (day, startMin, anchorRect) => {
    ...
    setQuickCreate({ noteId: id, anchorRect });
  };
...
function QuickCreatePopover({ note, folders, anchorRect, onCancel, onSave }) {
    ...
    <>
      ...
      <div className="quick-popover" style={{ left, top, width: POPUP_W }} onMouseDown={(e) => e.stopPropagation()} onKeyDown={handleKey}>
        ...
        <div className="quick-popover-actions">
          ...
          <button type="button" className="qp-btn qp-btn-primary" onClick={() => onSave({ title, body, folderId })}>Save</button>
        </div>
      </div>
    </>
  );
...
function AllDayBlock({ note, color, leftPct, widthPct, topPx, days, onSchedule, onUnschedule, onEditNote }) {
  const [drag, setDrag] = useState(null); // { mode: 'move'|'resize-left'|'resize-right', curX, curY, offsetX, offsetY, widthPx, duration }
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      if (drag.mode === "resize-right") {
        ...
        // move within all-day row
        if (idx != null) {
          const newStartIdx = Math.max(0, Math.min(7 - duration, idx));
  ...
  return (
    <div
      ...
      onDoubleClick={(e) => { e.stopPropagation(); onEditNote && onEditNote(); }}
  ...
  return (
    <div className="month-grid">
      <div className="month-headers">
        {DAY_NAMES_LONG.slice(1).concat([DAY_NAMES_LONG[0]]).map((d, i) => (
    ...
    <div className="modal-backdrop" onClick={onCancel}>
      <div className="modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-actions">
          ...
          <button className="btn-danger" onClick={onConfirm}>{confirmText || "Delete"}</button>
        </div>
      </div>
    </div>
  );
...
function App() {
  ...
  const requestConfirm = (cfg) => setConfirmCfg(cfg);
  ...
  return (
    <div className="app-root">
      ...
      />
      {state.tab === "todo"
        ? <TodoView state={state} setState={applyState} search={search} requestConfirm={requestConfirm} togglePinActive={togglePinActive} onOpenTrash={() => setTrashOpen(true)}/>
        : <CalendarView state={state} setState={applyState} search={search} requestConfirm={requestConfirm} onOpenTrash={() => setTrashOpen(true)}/>}
      <ConfirmModal
        open={!!confirmCfg}
